...
class DS1302:
    ...
    This class is used unmodified from the original state, except to include it directly in the main.py code instead of a separate module.
    The reason for this election is that when used in a separate module, functions such as (namely) date_time() were returning errors as 
    being undefined, which they clearly are. I can only assume this is a RAM overflow issue, as when the code is included directly 
    instead of imported, these errors do not occur.
    
    As this is not a class of my own creation, it does not have any of my conventions in naming or commenting.
    
    Additionally: the original file contents *are* stored in a ds1302.py module, for easy update and/or cross-compare if ever needed.'''
...
class Device_Info:
	...
	def __init__(self):
        ...
        self.<device name> = PWM_Dev(
            ...
            <A capitalized string in quotes, using one of the 5: All, Winter, Spring, Summer, Autumn. The start of the season activates the device>,
            
            <A capitalized string in quotes, using one of the 5: All, Winter, Spring, Summer, Autumn. The start of the season deactivates the device>,
            
            <A boolean value; True activates the device at the highest trigger-tempreature and deactivates it at the lowest trigger-temprature; False reverses this arrangement>
            )'''

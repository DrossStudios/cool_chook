...
class PWM_Dev():
	...
	def time_ref(self, time_in):
		if self.season_on == Coop.season or self.season_on == "all":
			if self.dev_on == "24": # all-day, everyday during this season
				self.pin_id.duty_u16(65535)
			elif self.dev_on == Coop.day_cycle and self.fade_count <= self.dev_off: # The time is "now" but the fade is still progressing
				...
				pass
			elif self.dev_on == Coop.day_cycle and self.fade_count > self.dev_off: # The time is "now" and the fade is done
				self.pin_id.duty_u16(65535)
			elif self.dev_on != Coop.day_cycle and self.fade_count > 0: # The time is "not now" but the fade needs to process
...
while True:
	...
	LogMe.poll_log(Coop, Temp.therm, pull_now)
	#Coop.poll_dev(Temp.therm, pull_now)
